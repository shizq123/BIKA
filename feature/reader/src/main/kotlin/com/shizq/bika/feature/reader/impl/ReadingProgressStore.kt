 ...
 * 将阅读进度持久化到数据库。
 *
 * [store] 采用**同步阻塞写库**（runBlocking + IO）：调用方线程（主线程）会短暂等待写库完成，
 * 保证返回时数据已落库。这消除了对 ApplicationScope/异步队列的依赖，
 * 即使返回/退后台后进程被立即回收，进度也不会丢失；任何写库异常也会当场捕获并提示。
 *
 * 常规翻页的自动保存应使用 [store]（协程内异步写库，不阻塞主线程）；
 * 仅在返回、退后台、销毁等关键时机使用同步 [store]。
 */
@Singleton
class ReadingProgressStore @Inject constructor(
    ...
    suspend fun store(
        ...
        return try {
            withContext(Dispatchers.IO) {
                storeInternal(comicId, chapterOrder, effectiveMeta, pageIndex)
            }
    ...
    private suspend fun storeInternal(
        comicId: String,
        chapterOrder: Int,
        meta: ChapterMeta,
        pageIndex: Int
