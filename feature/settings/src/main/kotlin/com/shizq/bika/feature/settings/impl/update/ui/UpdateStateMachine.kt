...
class UpdateStateMachine @Inject constructor(
        ...
        spec {
            inState<UpdateUiState.Idle> {
                on<UpdateAction.CheckUpdate> { action ->
                    override { UpdateUiState.Checking(source = action.source) }
                }
            ...
            inState<UpdateUiState.Checking> {
                onEnter {
                    val result = checkUpdate(source = snapshot.source)
            ...
            inState<UpdateUiState.Downloading> {
                ...
                collectWhileInState(downloadProgress) { progress ->
                    mutate { UpdateUiState.Downloading(release, progress = progress) }
                }
            }
                ...
                on<UpdateAction.Retry> { action ->
                    when (val retry = action.retryAction) {
                        is UpdateRetryAction.CheckUpdate -> {
                            override { UpdateUiState.Checking(source = retry.source) }
                        }
    ...
    private suspend fun checkUpdate(
        ...
        return try {
            val release = checkAppUpdateUseCase() ?: return UpdateUiState.NoUpdate(source = source)
